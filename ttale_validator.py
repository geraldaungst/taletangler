...
def validate_story(story: Story, story_parser: StoryParser, start_scene: str) -> bool:
    validated = confirm_endings(story, story_parser)
        ...
        for choice in scene.choices:
            if choice.next_scene == scene_tag:
                ...
                story_parser.notes.append(errors.TTError(
                    ...
                    scene_tag,
                    None,  # Cannot ascertain line number of self-loop
                    errors.ErrText.SELF_LOOP.value
                ))
    return validated     # Will eventually return False if any problems are found
